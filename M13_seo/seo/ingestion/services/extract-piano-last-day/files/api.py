...
class AnalyticsPlatformAPI:
    ...
    def pagination_get_data(self, query_template, max_retries=3, retry_delay=3, max_consecutive_errors=5):
        page_number = 1
        data = []
        ...
        while page_number <= 20:
            ...
            while retries < max_retries:
                try:
                    ...
                    retries += 1
                    logging.error(f"Error fetching data on page {page_number} (attempt {retries}/{max_retries}): {e}")
                    if retries >= max_retries:
                        logging.error(f"Max retries reached for page {page_number}. Skipping to the next page.")
                        page_number += 1
                        consecutive_errors += 1
                        if consecutive_errors >= max_consecutive_errors:
                            logging.error("Consecutive errors limit reached. Returning data fetched so far.")
                            return data
                    else:
