...
def main():
    ...
    account_gsc_sa = json.loads(get_secret(os.getenv("GSC_SA"), os.getenv("GCP_SECRET_PROJECT_ID")))
    account_bq_table = os.getenv("BQ_TABLE_GSC")

    bq_client = BigQuery(service_account_info=DATA_POOL_BIGQUERY_SA, project_id=os.getenv("DATA_POOL_PROJECT_ID"))
        ...
        for fetch_date in sorted(unfinalized_dates):
            ...
            for site_url in url_properties:
                for search_type in SEARCH_TYPES:
                    rows = gsc_api.fetch_data(site_url, str(fetch_date), str(fetch_date), search_type, data_state="final")
                    if rows:
                        logging.info(f"Fetching FINAL data for {fetch_date} [{search_type}]")
                        for row in rows:
                            keys = row["keys"]
                            row_data = {
                                "Date": keys[0],
                            ...
                            if search_type == "Discover":
                                row_data["Country"] = keys[1]
                                row_data["DeviceCategory"] = None
                            else:
                                row_data["DeviceCategory"] = keys[1]
                                row_data["Country"] = keys[2]
            ...
            if all_data:
                ...
                bq_client.delete_rows_by_dates(dataset, account_bq_table, df["Date"].dt.date.unique())
    ...
    start_date = (max_date + timedelta(days=1)) if max_date else today - timedelta(days=MAX_DATA_AGE_DAYS)
    ...
    while start_date <= today:
        for site_url in url_properties:
            for search_type in SEARCH_TYPES:
                ...
                rows = gsc_api.fetch_data(site_url, str(start_date), str(start_date), search_type, data_state="all")
                all_data = []
                if rows:
                    for row in rows:
                        keys = row["keys"]
                ...
                if all_data:
                    if str(start_date) in last_3_days:
                        bq_client.delete_rows_by_dates(dataset, account_bq_table, [str(start_date)], search_type)
